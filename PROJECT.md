...
## Phase 8+ — Cutover tracks: custom domain + TWA (в ход)

Двата последни roadmap елемента са **започнати**: repo-частта е завършена и
верифицирана; остатъкът е account-side (registrar / Vercel / Google / Play).
...
- ✅ **Код готов** — `/.well-known/assetlinks.json` endpoint (`src/app/api/assetlinks/route.ts` + rewrite в `next.config.ts`), env-driven (`TWA_PACKAGE_NAME` + `TWA_SHA256_FINGERPRINTS`); връща 404 докато не е конфигуриран; runtime-проверен (`curl` → HTTP 200 + валиден Digital Asset Links JSON)
- ⏳ **Чака**: Bubblewrap init/build (JDK 17 + Android SDK) → signing key → SHA-256 fingerprint-и във Vercel env → redeploy → Play Console (internal testing → prod). Build срещу `insulin-reset.bg` (asset links са origin-exact) → след domain cutover-а
